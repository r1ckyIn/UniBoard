...
- [x] **OPS-01**: GitHub Actions CI pipeline with separate backend (mypy+ruff+pytest) and frontend (tsc+eslint+build) workflows
- [x] **OPS-02**: Railway and Vercel deployment config (railway.toml, vercel.json, env var documentation), deployed to production
- [x] **OPS-03**: Sentry integrated for both Python backend (sentry-sdk) and Next.js frontend (@sentry/nextjs) with error tracking and performance monitoring
