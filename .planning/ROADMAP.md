...
| 33. Token Lifecycle & Onboarding | v3.0 | 0/8 | Not started | - |
...
Plans:
- [ ] 33-01-PLAN.md — Migration: profiles.recall_email_sent_at + handle_new_user trigger COALESCE patch (EMAIL-03, AUTH-HARDEN-01)
- [ ] 33-02-PLAN.md — RecallEmailService + SES extension + check_token_health() integration (EMAIL-03)
- [ ] 33-03-PLAN.md — /sync/status response: per_platform_counts schema + frontend types regen (ONBD-01)
- [ ] 33-04-PLAN.md — Supabase Google OAuth config + /auth/callback Next.js route (AUTH-HARDEN-01)
- [ ] 33-05-PLAN.md — Google button on LoginForm/RegisterForm + USYD banner + check-email copy fix (AUTH-HARDEN-01, AUTH-HARDEN-02)
- [ ] 33-06-PLAN.md — Resend email button + 60s cooldown on ForgotPasswordForm (AUTH-HARDEN-03)
- [ ] 33-07-PLAN.md — Setup edge cases: TokenStep cache + invalid/unreachable UX + SuccessStep platform rows + retry-failed (ONBD-01, ONBD-02)
- [ ] 33-08-PLAN.md — Documentation: TRD §7.X + §16.X + PROJECT.md Key Decisions + supabase/config.toml comment (AUTH-HARDEN-04)
