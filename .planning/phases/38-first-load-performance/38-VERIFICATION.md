...
| 2 | On cached-auth revisit (JWT valid, warm Railway), first paint shows real data across the 6 pages — no `SkeletonCard` visible | VERIFIED (re-verified 2026-04-22 post Phase 38.2) | Production naked-eye UAT on 2026-04-22 confirmed `pass` after Phase 38.2 (`0e21f64`) + warmup API URL fix (`89735c2`) deployed. Original Phase 38 contract was insufficient — Phase 38.1 closed the prefetch-consumer-parity gap, then Phase 38.2 removed `force-dynamic` + deleted `loading.tsx` + added `experimental.staleTimes.dynamic: 30` + activated Railway warmup cron. Full chain skeleton-free UX now functional. See `.planning/phases/38-first-load-performance/38-HUMAN-UAT.md` Test #1 for the flipped result. |
...
| 4 | Railway cold-start behaviour characterised — p50/p95 measured + warmup decision | VERIFIED (re-verified 2026-04-22 post Phase 38.2) | Decision = ACTIVATE WARMUP, made architecturally from production UAT evidence (2-3s skeleton duration on URL-entry, documented in `.planning/debug/resolved/sidebar-nav-skeleton-stall.md`). Phase 38.2 activated the cron (`*/10 * * * *`) + fixed `/healthz → /health` + fixed missing `NEXT_PUBLIC_API_URL` secret with hardcoded fallback (`89735c2`). Manual workflow_dispatch of railway-warmup.yml post-fix returned success in 9s. Formal 150-min p50/p95 measurement deferred as a nice-to-have optimization-validation item; the warmup decision itself is closed. See `.planning/phases/38-first-load-performance/coldstart-report.md` for activation record and `.planning/phases/38.2-navigation-cache-parity/` for the correction phase artifacts. |
...
**Score:** 9/10 fully verified / 1 human-needed (Playwright baseline capture Task 38-04-05 still gated on Supabase credential checkpoint) = **9/10 automatable must-haves**. Score upgraded from 7/10 → 9/10 on 2026-04-22 after Phase 38.2 landed the architectural correction and production UAT confirmed the skeleton-free + warmup-active state. Only Truth #5 (ROADMAP 999.2 verdict flip) remains PARTIAL, still gated on the 38-04-05 Playwright baseline capture.
