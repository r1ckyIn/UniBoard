...
| MOTION-01 | 03 | 2 | Zero matches for `transition-(all\|colors)\s+duration-(\d+\|\[)` in `frontend/{app,components}` | — | unit (grep wrapper) | `pnpm test -- --run __tests__/lint/no-raw-transition.test.ts` | ❌ W0 |
...
| MOTION-01 | 03 | 2 | Playwright pixel-diff: 10-page interaction state snapshots match v2.0 baseline (≤ 0.5% per page) | — | visual regression | `pnpm playwright test --grep "@phase39 @transition-parity"` | ❌ W0 |
