...
async def get_course_materials(
    ...
    result = await svc.get_course_materials(current_user.id, course_id)
...
async def get_folder_items(
    ...
    result = await svc.get_folder_items(current_user.id, course_id, folder_id)
    return SuccessResponse(data=result, meta=get_request_meta(request))
...
async def search_materials(
    ...
    result = await svc.search(current_user.id, q)
    return SuccessResponse(data=result, meta=get_request_meta(request))
